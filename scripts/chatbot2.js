// Google Gemini API Chatbot 2 - Direct API Implementation
...
async function callAI(prompt, base64Image, lang){
  ...
  const systemPrompt = `You are an expert Agronomy Assistant. Answer directly and helpfully in ${language}. Use any provided crop/soil/weather/telemetry context. 
  
Format your responses in a structured, organized way similar to ChatGPT:
1. Use clear paragraph breaks between different topics
2. Use headings (h3, h4) for main sections when appropriate
3. Use bullet points or numbered lists for steps or related items
4. Bold important terms or key points using <strong> tags
5. Create clear visual separation between sections
6. Use proper HTML formatting for clarity and readability
...
function typeAssistant(html){
  ...
  appendMsg('assistant', formatAIResponse(html));
}

// Format AI response with better structure
function formatAIResponse(html) {
  // Replace simple newlines with proper paragraph breaks
  ...
  // Fix any potential issues with nested paragraphs
  formatted = formatted.replace(/<p><p>/g, '<p>').replace(/<\/p><\/p>/g, '</p>');
  
  // Enhance headings with proper styling
  formatted = formatted.replace(/<h3>/g, '<h3 class="ai-heading">').replace(/<h4>/g, '<h4 class="ai-heading">');
  
  // Enhance lists with proper styling
  formatted = formatted.replace(/<ul>/g, '<ul class="ai-list">').replace(/<ol>/g, '<ol class="ai-list">');
  
  // Add custom styling if not already present
  if (!document.querySelector('.ai-response-styles')) {
    const styleEl = document.createElement('style');
    styleEl.className = 'ai-response-styles';
    styleEl.textContent = `
      .ai-heading {
        margin: 16px 0 8px 0;
